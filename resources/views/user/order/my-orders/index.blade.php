                                ...
                                <td>
                                    @php
                                        ...
                                        if (!$order->payments->isEmpty()) {
                                            ...
                                            } elseif ($order->payments->last()->type === 'fullpayment' && ! $order->payments->last()->is_verified) {
                                                $badgeClass = 'text-bg-warning';
                                                $paymentStatus = 'Full Payment Verification';
                                            } elseif ($order->payments->last()->type === 'fullpayment' && $order->payments->last()->is_verified) {
                                                $badgeClass = 'text-bg-success';
                                                $paymentStatus = 'Paid';
                                            } elseif ($order->payments->first()->type === 'downpayment' && !$order->payments->first()->is_verified) {
                                                $badgeClass = 'text-bg-warning';
                                                $paymentStatus = 'Down Payment Verification';
                                            } elseif ($order->payments->last()->type === 'downpayment' && $order->payments->first()->is_verified) {
                                                $badgeClass = 'text-bg-info';
                                                $paymentStatus = 'Down Payment Verified';
                                            } elseif ($order->payments->last()->type === 'balance' && ! $order->payments->last()->is_verified){
                                                 $badgeClass = 'text-bg-warning';
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                            ...
                                            @if ($order->status === 'done' && $order->payment_status === 'Payment Settled')
                                                    <li>
                                                        <a class="dropdown-item" href="{{ route('user.order.mark-as-complete', [$order]) }}">Mark
                                                            as Completed</a>
                                                    </li>
                                            @endif
